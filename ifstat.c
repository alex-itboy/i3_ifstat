...
#define LEN(options, namelen)                                                  \
  (((options)&OPT_FIXEDWIDTH || (namelen) < WIDTH) ? WIDTH : (namelen))
...
static void print_stats(struct ifstat_list *ifs, struct timeval *start,
                        struct timeval *end, int options) {
  ...
  //     fprintf(stdout, "%02d:%02d:%02d" SPACE, ltm->tm_hour, ltm->tm_min,
  //             ltm->tm_sec);
  //   else
  //     fputs("--:--:--" SPACE, stdout);
  // } else if (options & OPT_WRAP)
  ...
  delay = end->tv_sec - start->tv_sec +
          ((double)(end->tv_usec - start->tv_usec)) / (double)1000000;
...
int main(int argc, char **argv) {
  ...
  char *dname = NULL;
  char *dopts = NULL;
  double delay = 0.99, first_delay = 0.99;
  int count = 0;
  int options = 0;
  ...
  //# make sure user select one interface
  if (!(options && OPT_NOSCAN)) {
    fprintf(stderr, "Must have a interface provied(use argument -i)\n");
    exit(EXIT_FAILURE);
  }
  ...
  //# don't need loop for it will only output once
